package Fishing1_4;
	...
	static int money=51000; //수중에 가지고 있는돈 초기값 50000
	public static int foodnum; //먹이 갯수
	public static String myrodName; //낚싯대 이름 변수
	static int l=0; //잡은물고기 배열 변수
	...
	public void enterShop() {//상점초기메뉴
			...
			if(input==2) { //2를 입력했을때 2
				...
				else if(input2==2) { // 2입력후 2를 입력헀을때 2-2
					if(money<45000) {
						System.out.println("money가 부족합니다.");
					}
					else {
				...
				else if(input2==3) { // 2입력후 3을 입력했을때 2-3
					if(money<49000) {
						System.out.println("money가 부족합니다.");
						System.out.println();
					}else {
				...
				else if(input2==4) { // 2입력후 4를 입력했을때 2-4 
					enterShop();
				}
				else {
			...
			if(input==3) { //3입력했을때 3
				...
				input2 = sc.nextInt();
				if(input2==1) { // 3입력후 1입력했을때 3-1
					fishes fishes=new fishes();
					//내 수족관에 있는 물고기 정보를 가져온다.
					System.out.println(log.necname+"님이 보유한 물고기");
					while(l<fishes.num) {
					System.out.println("물고기 길이: "+fishes.fishcasew[l].fishlength+"cm |물고기 종류: "+fishes.fishcasew[l].fishname+" |물고기 시세: "+fishes.fishcasew[l].fishprice+"원");
					l++;
					}//물고기들의 이름과 가격정보를 보여준다 
					System.out.println("모두 판매하시겠습니까? Yes:1번");
					System.out.println("상위 메뉴 Go:2번");
					System.out.print(">>");
					input2 = sc.nextInt();
					if(input2==1) { //3입력후 1입력하고 1입력했을때 3-1-1
					...
					if(input2==2) { //3입력후 2입력했을때 3-2
						...
						System.out.print(">>");
						input2 = sc.nextInt();
						if(input2==1)  { //3입력후 2입력하고 1입력했을때 3-2-1 
							try {
							if(!(myrodName.equals(null))) { //myrodName이 null이라면
								myrodName=null;
								money += 20000;

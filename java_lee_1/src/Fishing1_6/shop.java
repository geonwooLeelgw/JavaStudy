package Fishing1_6;
	...
	public class shop {
		...
		static int money=50000; //수중에 가지고 있는돈 초기값 50000
...
System.out.println("1.먹이 구입"+"  "+"2.낚시대 구입"+"  "+"3.판매"+"  "+"4.상점 나가기");
			...
			if(input==3) {
				...
				if(input2==1) {
					...
					System.out.println(log.necname2+"님이 보유한 물고기");
					while(l<fishes.num) {
					...
					if(input2==1) {
						...
						if(l==fishes.num && fishes.price==0) {
						enterShop();
						}
						l=0;
						enterShop();
...
while(input!=4);{
	if(log.num2!=4) {
			shoplist fishshop2=new shoplist(log.necname2, money, myrodName, food);
